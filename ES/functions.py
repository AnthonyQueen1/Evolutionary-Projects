...
def fitness(ind):
    ...
    if isinstance(ind, individual):
        ind = ind.parameters
    return (abs(ind[0])+abs(ind[1]))*(1+abs(math.sin(3*abs(ind[0])*math.pi))+abs(math.sin(3*abs(ind[1])*math.pi)))
    # return (abs(ind[0])+abs(ind[1]))*(1+abs(math.sin(abs(ind[0])*math.pi))+abs(math.sin(abs(ind[1])*math.pi)))
